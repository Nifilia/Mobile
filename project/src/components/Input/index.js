...
const Input = ({label, placeholder, type, options, isPassword, value, onChangeText, style, ...props}) => {
    ...
    return (
        <View style={styles.container}>
            ...
            {type === 'picker' ? (
                <Pressable onPress={() => setPickerModalVisible(true)} style={styles.inputContainer}>{ value ? ( 
                    <Text style={[styles.placeholder, style]}>{value?.title}</Text> ) : ( <Text style={[styles.placeholder, style]}>{placeholder}</Text>)}
                ...
                <Modal transparent visible={isPickerModalVisible}>
                    <TouchableOpacity activeOpacity={1} onPress={() => setPickerModalVisible(false)} style={styles.modalWrapper}>
                        <TouchableOpacity activeOpacity={1} style={styles.modalContent}>
                            <Text style={styles.optionsTitle}>Select options</Text>
                            {
                                options?.map( opt => {
                                    if(!opt?.id) {
                                        return null;
                                    ...
                                    return (<Text onPress={() => onSelect(opt)} style={[styles.optionsText, selected ? styles.selectedOption : {}]} key={opt?.title}>{opt?.title}</Text>)
                                })}
                            </TouchableOpacity>
                        </TouchableOpacity>
                    </Modal>
